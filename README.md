# AI Transformation Lab

A free, open-source, tool-agnostic AI upskilling program for professionals.

**No installation. No server. No cost. Open it in any browser.**

→ **[Launch the lab](https://mikeadediran.github.io/aiLab/)**

---

## What it is

The AI Transformation Lab is a hands-on learning experience designed to help professionals — regardless of technical background — understand AI, apply it to real work problems, and build the habits that make the gains stick.

It is not a slide deck. It is not a course. It is a live workspace you open in a browser and work through with real problems you bring from your own job.

It was built for:
- **Professionals at any level** who want to use AI to work smarter
- **Facilitators and champions** running upskilling programs inside organisations
- **Self-paced learners** going through it independently
- **Anyone** who wants to run this with their own team or community

---

## Structure

The program runs across four sessions. Sessions 1–3 are for everyone. Session 4 is optional — for those who want to go further and start building.

| Session | Focus | Duration |
|---------|-------|----------|
| 1 | Foundation + problem framing | 60–90 min / session |
| 2 | Applied lab + prompt mastery | 60–90 min / session |
| 3 | Showcase + champion toolkit | 60–90 min / session |
| 4 | Builder track (optional) | 90 min+ / session |

**Built-in tools:**
- Prompt lab with RCTFX framework
- Wins wall for cohort progress tracking
- Facilitator notes per session (toggle with "I'm a facilitator")
- Session timer
- ROI capture and export
- Presentation mode for wins wall
- Group and self-paced modes
- Works offline after first load

---

## How to use it

### As a facilitator running a cohort

1. Open the URL on your machine and project it, or share the link so participants open it on their own devices
2. Click **"I'm a facilitator"** to access session notes and the timer
3. Set the mode to **"Attending a group session"**
4. Follow each session tab in order — the interface guides the session
5. Participants log wins to the Wins Wall in real time
6. At the end, everyone downloads their session data from the Wins Wall

**Recommended cadence:** One session per week over three weeks, then an optional fourth session for self-selected builders.

### As a self-paced learner

1. Open the URL in any browser
2. Set the mode to **"Learning on my own"**
3. Work through each session at your own pace
4. Use the wins journal to track what's working
5. Download your session data at the end

---

## Running it locally

No build step. No dependencies. No install.

```bash
git clone https://github.com/mikeadediran/aiLab.git
cd aiLab
open index.html
```

Or just download `index.html` and open it in any browser.

---

## Tool agnostic

This program works with any AI tool — ChatGPT, Claude, Gemini, Copilot, Perplexity, or your organisation's enterprise AI. It does not integrate with or require any specific tool.

---

## Contributing

Improvements, bug fixes, and new worked examples are welcome. See [CONTRIBUTING.md](CONTRIBUTING.md) for how to get involved.

---

## License

MIT — free to use, adapt, and share. Attribution appreciated but not required.

---

## Creator

Questions, feedback, or want to share how you used this? Reach out at [mikeadediran11@gmail.com](mailto:mikeadediran11@gmail.com) or connect on [LinkedIn](https://linkedin.com/in/mikeadediran).
